...
export const voiceApi = {
  ...
  sendCallInvite(input: { channelId: string; kind: 'audio' | 'video' }) {
    return invoke<void>('send_call_invite', { input });
  },
  cancelCallInvite(input: { channelId: string; kind: 'audio' | 'video' }) {
    return invoke<void>('cancel_call_invite', { input });
  },
  broadcastVoiceState(input: {
    channel_id: string;
    is_muted: boolean;
    is_deafened: boolean;
    ...
    return invoke<void>('broadcast_voice_state', { input });
  },
  requestVoicePresence() {
    return invoke<void>('request_voice_presence');
  },
...
export const fileApi = {
  ...
  getDataUrl(
    ...
    return invoke<string>('get_file_data_url', {
      fileId,
      r2Key: r2Key ?? null,
      contentKeyCiphertext: contentKeyCiphertext ?? null,
