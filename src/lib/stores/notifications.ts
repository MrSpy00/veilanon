...
let activeRingStop: (() => void) | null = null;
...
export const toastStore = {
  ...
  /**
   * Incoming call ring: looping chime + desktop notification
   */
  async notifyIncomingCall(params: { callerName: string; kind: 'audio' | 'video' }) {
    stopIncomingRing();
    const settings = await getOrFetchSettings();
    const soundMasterEnabled = settings?.notificationSound ?? true;
    const callSoundEnabled = settings?.soundCalls ?? true;
    const isDnd = isDndActive(settings);
    if (soundMasterEnabled && callSoundEnabled && !isDnd) {
      activeRingStop = playIncomingCallRing(computeVolume(settings, 0.5));
    }

    notificationStore.add({
      type: 'call',
      title: params.kind === 'video' ? `📹 Görüntülü Arama — ${params.callerName}` : `📞 Sesli Arama — ${params.callerName}`,
      body: `${params.callerName} seni arıyor…`,
      username: params.callerName,
    });

    if (settings?.desktopNotifications && !isDnd && !isStreamerPopupSuppressed()) {
      const hasPerm = await checkDesktopPermission();
      if (hasPerm) {
        try {
          sendNotification({
            title: params.kind === 'video' ? 'Görüntülü Arama' : 'Sesli Arama',
            body: `${params.callerName} seni arıyor…`,
          });
        } catch {
          // best-effort
