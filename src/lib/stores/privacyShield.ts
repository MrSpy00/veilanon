...
export const privacyShield = {
  ...
  isRevealedReactive(key: string): boolean {
    const map = get(revealedSecrets);
    const v = map[key];
    return typeof v === 'number' && v > Date.now();
  },
  ...
  formatSecret(value: string | null | undefined, key: string, forceMask = false): string {
    if (!value) return '';
    if (privacyShield.isSecretRevealed(key)) {
      return value;
    }
    if (forceMask) {
      const cfg = get(configStore);
      const style = cfg.maskStyle;
      if (style === 'bullets') return '•'.repeat(Math.min(Math.max(value.length, 8), 24));
      if (style === 'asterisks') return '*'.repeat(Math.min(Math.max(value.length, 8), 24));
      if (style === 'hidden') return '[GİZLENDİ]';
      return '••••••••••••';
    }
    const cfg = get(configStore);
    const shieldActive = get(isShieldActive);
    const streamer = get(streamerMode);
    const shouldMask = shieldActive || streamer.enabled;
    if (!shouldMask) {
      const autoHide = cfg.autoHideTimeoutSeconds;
      if (autoHide === 0) return value;
      return value;
    }
    const style = cfg.maskStyle;
    if (style === 'bullets') {
  ...
  formatSecretManual(value: string | null | undefined, key: string): string {
    if (!value) return '';
    if (privacyShield.isSecretRevealed(key)) return value;
    const cfg = get(configStore);
    const style = cfg.maskStyle;
    if (style === 'bullets') return '•'.repeat(Math.min(Math.max(value.length, 8), 24));
    if (style === 'asterisks') return '*'.repeat(Math.min(Math.max(value.length, 8), 24));
