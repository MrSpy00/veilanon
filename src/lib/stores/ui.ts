...
function createUiStore() {
  ...
  function refreshDomTheme(state: UiState) {
    ...
    document.documentElement.style.setProperty('--veil-bg-brightness', `${state.customBgBrightness ?? 1.0}`);
  ...
  return {
    ...
    loadUserTheme(userId: string | null) {
      ...
      const storedBrightness = parseFloat(localStorage.getItem(userBgKey('veilanon-bg-brightness', userId)) || '1.0');
      ...
      update(s => {
        const next = {
          ...
          customBgBrightness: isNaN(storedBrightness) ? 1.0 : Math.max(0.2, Math.min(2.5, storedBrightness)),
    ...
    setCustomBackground(image: string, video: string, opacity: number, brightness?: number) {
      const clampedOpacity = Math.max(0, Math.min(1.0, opacity));
      const clampedBrightness = brightness !== undefined ? Math.max(0.2, Math.min(2.5, brightness)) : undefined;
      update(s => {
        const next: UiState = {
          ...s,
          customBgImage: image,
          ...
          customBgBrightness: clampedBrightness !== undefined ? clampedBrightness : (s.customBgBrightness ?? 1.0),
        };
        ...
        if (clampedBrightness !== undefined) localStorage.setItem(userBgKey('veilanon-bg-brightness'), String(clampedBrightness));
        refreshDomTheme(next);
        return next;
      });
    },

    setBgBrightness(brightness: number) {
      const clamped = Math.max(0.2, Math.min(2.5, brightness));
      update(s => {
        const next = { ...s, customBgBrightness: clamped };
        localStorage.setItem(userBgKey('veilanon-bg-brightness'), String(clamped));
        refreshDomTheme(next);
        return next;
      });
    },
    ...
    resetCustomLayer() {
      update(s => {
        ...
        localStorage.removeItem(userBgKey('veilanon-bg-brightness'));
    ...
    initTheme() {
      ...
      const storedBrightness = parseFloat(localStorage.getItem(userBgKey('veilanon-bg-brightness')) || '1.0');
