...
/** Ringing lifecycle: auto-dismiss after 45s if nobody answers/cancels. */
let incomingRingTimer: ReturnType<typeof setTimeout> | null = null;

const RING_TIMEOUT_MS = 45_000;

function showIncomingCall(info: IncomingCallInfo) {
  if (incomingRingTimer) clearTimeout(incomingRingTimer);
  incomingCall.set(info);
  void toastStore.notifyIncomingCall({
...
/** Accept the incoming call: navigate to its DM channel and join voice/video. */
export async function acceptIncomingCall(): Promise<void> {
  const info = get(incomingCall);
  dismissIncomingCall();
  if (!info) return;
  try {
    uiStore.navigateDm(info.channelId);
    const { mediaStore } = await import('./media');
    await mediaStore.joinVoice(info.channelId, info.kind === 'video');
    if (info.kind === 'video') {
      await mediaStore.toggleCamera().catch(() => {});
    }
  } catch {
    toastStore.error('Aramaya katılamadın.');
  }
}

/** Decline the incoming call — just closes the ring locally. */
...
// Precise server UTC seconds provider
...
function createMessageStore() {
  ...
  function handleCallSignal(payload: Record<string, unknown> | null | undefined) {
    const actual = ((payload?.payload || payload) as Record<string, unknown>) || {};
    const type = actual.type as string;
    if (type !== 'call_invite' && type !== 'call_cancel') return;

    let selfId: string | null = null;
    try {
      selfId = get(authStore).identity?.id ?? null;
    } catch { /* ignored */ }

    if (type === 'call_cancel') {
      const current = get(incomingCall);
      if (current && (actual.caller_id === current.callerId)) {
        dismissIncomingCall();
      }
      return;
    }

    const callerId = actual.caller_id as string;
    if (!callerId || !selfId || callerId === selfId) return;

    const channelId = actual.channel_id as string;
    if (!channelId) return;

    // Only show ringing notification if this call is meant for this user (DM / peer)
    try {
      const spaces = get(spaceStore);
      const isMyDm = spaces.dmChannels.some((d) => d.id === channelId || d.peerId === callerId);
      if (!isMyDm) return;
    } catch { /* best effort */ }

    const kind = actual.kind === 'video' ? 'video' : 'audio';
    const callerName =
      (actual.caller_name as string) ||
      (actual.caller_username as string) ||
      'Kullanıcı';
    ...
    showIncomingCall({
      ...
      callerId,
      callerName,
      avatarHash: (actual.avatar_hash as string) ?? null,
    });
  }

  function handleBroadcast(payload: Record<string, unknown>, selfStore: { syncChannel: (chId: string) => Promise<void> }) {
    const actual = ((payload?.payload || payload) as Record<string, unknown>) || {};
    const type = actual.type as string;
    ...
    if (
      ...
      const channelId = actual.channel_id as string;
      if (channelId) {
        void selfStore.syncChannel(channelId);
      }
      try {
    ...
    if (
      ...
      try {
        ...
        if (auth.identity && targetUserId === auth.identity.id) {
    ...
    if (type === 'space_updated' || type === 'space_created' || type === 'space_deleted') {
      try {
        void spaceStore.loadSpaces();
        const spaceId = actual.space_id as string;
        if (spaceId) {
          void spaceStore.loadChannels(spaceId);
        }
      } catch { /* best effort */ }
      return;
    }

    if (type === 'channel_created' || type === 'channel_updated' || type === 'channel_deleted') {
      try {
        const spaceId = actual.space_id as string;
        if (spaceId) {
          void spaceStore.loadChannels(spaceId);
  ...
  return {
    ...
    async initRealtime(activeChannel: () => string | null) {
      ...
      try {
        broadcastUnlisten = await listen<Record<string, unknown>>(
          'veilanon:broadcast',
          (event) => {
            handleBroadcast(event.payload, this);
            handleCallSignal(event.payload);
          }
        );
      } catch {
    ...
    async syncChannel(channelId: string) {
      try {
        ...
        const activeMerged = merged.filter(m => !m.disappearsAt || m.disappearsAt > nowSec);
        if (activeMerged.length !== merged.length) {
          hasChanges = true;
        }

        if (!hasChanges) return;

        activeMerged.sort((a, b) => (a.createdAt || 0) - (b.createdAt || 0));

        saveChannelCache(channelId, activeMerged);

        update(s => ({
          ...s,
          byChannel: {
            ...s.byChannel,
            [channelId]: activeMerged,
          },
        }));
        ...
        for (const msg of incomingForNotify) {
          ...
          // Hayalet bildirim engeli: ne içerik ne ek varsa (şifre çözülemedi /
          // boş satır) kullanıcıya zil çaldırma — "bildirim geldi ama mesaj yok" bug'ı.
          if (!text.trim() && !hasAttachments) continue;
          ...
          if (isCurrentActiveChannel && isDocumentFocused) continue;
      ...
      try {
        const sent = await invoke<Message>('send_message', {
          input: {
            ...
            replyToId,
            attachments,
            disappearSeconds: disappearSeconds ?? null,
          }
        });
      ...
      try {
        ...
        update(s => {
          ...
          if (beforeId) {
            ...
            finalMessages = [...messages, ...currentList].filter(m => {
              if (seen.has(m.id)) return false;
              seen.add(m.id);
              return true;
            });
          } else if (messages.length > 0) {
            // Merge instead of overwrite: keep local-only rows (in-flight sends
            // plus freshly synced remote rows the backend read may lag behind on)
            // so messages never visually disappear between loads.
            const merged = new Map<string, Message>(messages.map(m => [m.id, m]));
            for (const m of currentList) {
              if (merged.has(m.id)) continue;
              merged.set(m.id, m);
            }
            finalMessages = Array.from(merged.values());
          } else {
            finalMessages = currentList.length > 0 ? currentList : messages;
          }

          finalMessages.sort((a, b) => (a.createdAt || 0) - (b.createdAt || 0));
