...
import { getPresetById, type ThemeTokens } from '$lib/themes/presets';
...
export function exportThemeJson(data: ThemeExportData): string {
  const exportPayload: ThemeExportData = {
    ...
    customBgBrightness: data.customBgBrightness ?? 1.0,
...
export function importThemeJson(jsonStr: string): { data: ThemeExportData | null; error: string | null } {
  try {
    ...
    const themeData: ThemeExportData = {
      ...
      customBgBrightness: typeof parsed.customBgBrightness === 'number' ? Math.max(0.2, Math.min(2.5, parsed.customBgBrightness)) : 1.0,
