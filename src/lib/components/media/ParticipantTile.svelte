<script lang="ts">
  ...
  function syncVideoAttachment() {
    ...
    if (isLocal) {
      ...
      const pub = lp?.getTrackPublication?.(Track.Source.Camera) ?? (lp ? Array.from((lp as any).videoTrackPublications?.values?.() || [])[0] : null) as any;
      ...
      if (track && track.kind === 'video') {
        ...
        if (!retryTimer) {
          ...
          retryTimer = setInterval(() => {
            ...
            const currPub = currLp?.getTrackPublication(Track.Source.Camera) ?? (currLp ? Array.from(currLp.videoTrackPublications.values())[0] : null);
      ...
      const camPub = p.getTrackPublication?.(Track.Source.Camera) ??
        (p.videoTrackPublications ? Array.from(p.videoTrackPublications.values())[0] : null);
      const camTrack = (camPub?.track ?? (camPub as any)?.videoTrack) as TrackType | undefined;
      const hasTrack = Boolean(camPub && camTrack && camTrack.kind === 'video' && !camPub.isMuted);
      remoteVideoActive = hasTrack;
      remoteMicActive = p.isMicrophoneEnabled;
      remoteSpeaking = p.isSpeaking;

      if (hasTrack && camTrack) {
        if (attachedTrack !== camTrack || !isTrackAttached) {
          if (attachedTrack && attachedTrack !== camTrack) {
            try { attachedTrack.detach(node); } catch { /* best effort */ }
          }
          attachedTrack = camTrack;
          try {
            camTrack.attach(node);
          } catch {
            if ((camTrack as any).mediaStreamTrack) {
              node.srcObject = new MediaStream([(camTrack as any).mediaStreamTrack]);
            }
          }
        }
        node.muted = false;
  ...
  function setupVideoElement(node: HTMLVideoElement) {
    ...
    if (p && typeof (p as any).on === 'function') {
      (p as any).on(ParticipantEvent.TrackSubscribed, onTrackEvent);
      (p as any).on(ParticipantEvent.TrackUnsubscribed, onTrackEvent);
      (p as any).on(ParticipantEvent.TrackMuted, onTrackEvent);
      (p as any).on(ParticipantEvent.TrackUnmuted, onTrackEvent);
      (p as any).on(ParticipantEvent.TrackPublished, onTrackEvent);
      (p as any).on(ParticipantEvent.TrackUnpublished, onTrackEvent);
      (p as any).on(ParticipantEvent.IsSpeakingChanged, onTrackEvent);
    }
    if (room && typeof room.on === 'function') {
      ...
      room.on(RoomEvent.TrackMuted, onTrackEvent);
      room.on(RoomEvent.TrackUnmuted, onTrackEvent);
      room.on(RoomEvent.ActiveSpeakersChanged, onTrackEvent);
    }
    ...
    return {
      ...
      destroy() {
        ...
        if (p && typeof (p as any).off === 'function') {
          (p as any).off(ParticipantEvent.TrackSubscribed, onTrackEvent);
          (p as any).off(ParticipantEvent.TrackUnsubscribed, onTrackEvent);
          (p as any).off(ParticipantEvent.TrackMuted, onTrackEvent);
          (p as any).off(ParticipantEvent.TrackUnmuted, onTrackEvent);
          (p as any).off(ParticipantEvent.TrackPublished, onTrackEvent);
          (p as any).off(ParticipantEvent.TrackUnpublished, onTrackEvent);
          (p as any).off(ParticipantEvent.IsSpeakingChanged, onTrackEvent);
        }
        if (room && typeof room.off === 'function') {
          ...
          room.off(RoomEvent.TrackMuted, onTrackEvent);
          room.off(RoomEvent.TrackUnmuted, onTrackEvent);
          room.off(RoomEvent.ActiveSpeakersChanged, onTrackEvent);
        }
        if (attachedTrack) {
          try { attachedTrack.detach(node); } catch { /* best effort */ }
          attachedTrack = null;
