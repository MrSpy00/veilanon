<script lang="ts">
  ...
  const status = $derived(full?.friendStatus ?? 'none');
  const isSelf = $derived(!!profile && profile.userId === $authStore.identity?.id);
  const onlineStatus = $derived(
    (isSelf ? $uiStore.presence : (full?.onlineStatus ?? profile?.onlineStatus ?? 'offline')) as 'online' | 'away' | 'dnd' | 'offline' | 'invisible'
  );
