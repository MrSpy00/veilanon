...
<script lang="ts">
  ...
  let bgBrightnessInput = $state(1.0);
  ...
  onMount(() => {
    ...
    bgBrightnessInput = ui.customBgBrightness ?? 1.0;
  ...
  function applyCurrentMedia() {
    ...
    if (detectedMediaType === 'video' || detectedMediaType === 'unknown') {
      uiStore.setCustomBackground('', url, bgOpacityInput, bgBrightnessInput);
      saveSettings({ customBgImage: '', customBgVideo: url, customBgOpacity: bgOpacityInput, customBgBrightness: bgBrightnessInput });
    } else {
      uiStore.setCustomBackground(url, '', bgOpacityInput, bgBrightnessInput);
      saveSettings({ customBgImage: url, customBgVideo: '', customBgOpacity: bgOpacityInput, customBgBrightness: bgBrightnessInput });
    }
  ...
  {:else if activeStudioTab === 'media'}
    <div class="veil-media-pane">
      ...
      <div class="veil-media-form">
        ...
        <div class="veil-form-group">
          ...
          <input
            ...
            oninput={() => {
              ...
              uiStore.setCustomBackground(img, vid, bgOpacityInput, bgBrightnessInput);
            }}
          />
        </div>
        ...
        <div class="veil-form-group">
          <div class="opacity-header">
            <label class="veil-form-label" for="bg-brightness-slider">Arka Plan Parlaklığı:</label>
            <span class="opacity-val">{Math.round(bgBrightnessInput * 100)}% {bgBrightnessInput > 1 ? '✨' : ''}</span>
          </div>
          <input
            ...
            step="0.05"
            class="veil-range-slider"
            style={`--range-pct: ${Math.round(((bgBrightnessInput - 0.2) / 2.3) * 100)}%`}
            bind:value={bgBrightnessInput}
            oninput={() => {
              uiStore.setBgBrightness(bgBrightnessInput);
            }}
          />
          <span class="veil-custom-hint">%100 normal, %200 ekstra parlak. Koyu görselleri aydınlatır, opaklıktan bağımsız çalışır.</span>
        </div>
