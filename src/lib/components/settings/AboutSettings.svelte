<script lang="ts">
  ...
  onMount(async () => {
    try {
      ...
      // Backend unavailable (browser preview) — fallback values stay.
    }
    try {
      diagnostics = await diagnosticsApi.get();
    } catch {
      // Diagnostics are Tauri-only.
    }
  });
  ...
  const statusRows = $derived(
    diagnostics
      ? [
          { label: 'Kontrol düzlemi (Supabase)', ok: diagnostics.supabaseConfigured && diagnostics.supabaseReachable },
          { label: 'Ses/Görüntü (LiveKit)', ok: diagnostics.livekitConfigured },
          { label: 'Blob depolama (R2)', ok: diagnostics.r2Configured },
          { label: 'Gerçek zamanlı bağlantı', ok: diagnostics.realtimeConnected },
        ]
      : [],
  );
</script>
        ...
        <div class="veil-diagnostics-status-grid">
          {#each statusRows as row (row.label)}
            <div class="veil-diag-card" class:ok={row.ok}>
              ...
              <span class="veil-diag-badge" class:ok={row.ok}>{row.ok ? 'Bağlı / Aktif' : 'Beklemede'}</span>
            </div>
          {/each}
        </div>
