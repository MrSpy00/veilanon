<script lang="ts">
  ...
  import { streamerMode, maskUserId, maskText } from '$lib/stores/streamerMode';
  ...
  <div class="veil-profile-hero">
    <div class="veil-profile-hero-banner">
      ...
      <div class="veil-banner-actions-overlay">
        <button class="btn btn-secondary btn-sm veil-banner-action-btn" type="button" onclick={changeBanner} disabled={avatarBusy}>
          <Icon name="image" size={13} />
          <span>{identity?.bannerHash ? 'Bannerı Değiştir' : 'Banner Ekle'}</span>
        </button>
        {#if identity?.bannerHash}
          <button class="btn btn-ghost btn-sm veil-banner-action-btn" type="button" onclick={removeBanner} disabled={avatarBusy}>
            Kaldır
          </button>
        {/if}
      </div>
    ...
    <div class="veil-settings-row">
      <div class="veil-settings-row-info">
        ...
        <div
          ...
          data-revealed={privacyShield.isSecretRevealed('device-id')}
        >
          {privacyShield.formatSecret(identity?.deviceId, 'device-id')}
        </div>
      </div>
      {#if identity?.deviceId}
        <div style="display:flex;gap:4px;">
          <button
            class="btn-icon"
            type="button"
            title={privacyShield.isSecretRevealed('device-id') ? 'Gizle' : '5 saniyeliğine göster'}
            onclick={() => privacyShield.toggleSecret('device-id', 5)}
          >
            <Icon name={privacyShield.isSecretRevealed('device-id') ? 'eye-off' : 'eye'} size={14} />
          </button>
          <button
            class="btn-icon"
            type="button"
  ...
  <div class="veil-settings-group">
    ...
    {#if fingerprint}
      <pre
        ...
        data-streamer-mask="id"
        data-auto-protect="secret"
        data-revealed={privacyShield.isSecretRevealed('fingerprint')}
      >{privacyShield.isSecretRevealed('fingerprint') ? formatFingerprint(fingerprint) : ($streamerMode.enabled || $isShieldActive ? '•••• •••• •••• •••• •••• •••• •••• ••••' : formatFingerprint(fingerprint))}</pre>
      <div style="display:flex;gap:8px;margin-top:4px;">
        ...
        >
          <Icon name={privacyShield.isSecretRevealed('fingerprint') ? 'eye-off' : 'eye'} size={14} />
          {privacyShield.isSecretRevealed('fingerprint') ? 'Gizle' : '5 sn Göster'}
        </button>
        <button
          class="btn btn-secondary btn-sm"
          type="button"
