...
impl RatchetState {
    ...
    /// Encrypt a message — advances send ratchet. Returns header, ciphertext, nonce, and derived message key.
    pub fn encrypt_with_key(&mut self, plaintext: &[u8]) -> VeilResult<(MessageHeader, Vec<u8>, Vec<u8>, [u8; 32])> {
