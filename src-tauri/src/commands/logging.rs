...
pub async fn get_diagnostics(state: State<'_, AppState>) -> VeilResult<Diagnostics> {
    ...
    let supabase_configured = config::configured("VEILANON_SUPABASE_URL") && config::configured("VEILANON_SUPABASE_ANON_KEY");
    let supabase_reachable = if supabase_configured {
        ...
        if let Ok(c) = client {
            if let Some(url) = config::var("VEILANON_SUPABASE_URL") {
                let health_url = format!("{}/auth/v1/health", url.trim_end_matches('/'));
                c.get(&health_url).send().await.map(|r| r.status().is_success()).unwrap_or(true)
            } else { true }
        } else { true }
    } else { false };
    let livekit_configured = config::configured("VEILANON_LIVEKIT_URL") && config::configured("VEILANON_LIVEKIT_API_KEY") && config::configured("VEILANON_LIVEKIT_API_SECRET");
    let r2_configured = config::configured("VEILANON_R2_ACCOUNT_ID") && config::configured("VEILANON_R2_BUCKET");
