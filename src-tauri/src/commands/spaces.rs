...
pub async fn invites_redeem(code: String, state: State<'_, AppState>) -> Result<SpaceInfo, VeilError> {
    ...
    // 4. Yerel üyeliği ekle
    {
        ...
        let space_present = db.get_space(&space_id).map(|s| s.is_some()).unwrap_or(false);
        if !space_present {
            let _ = db.insert_space(&space_id, "Topluluk", None, &identity.id);
        }
        let _ = db.add_space_member(&space_id, &identity.id);
    }
    ...
    let db = state.db.read().await;
    let row = match db.get_space(&space_id)? {
        Some(row) => row,
        None => {
            let _ = db.insert_space(&space_id, "Topluluk", None, &identity.id);
            db.get_space(&space_id)?.ok_or(VeilError::InvalidInput("Topluluk bulunamadı".into()))?
        }
    };
    let _ = state.app.emit("spaces:changed", ());
...
pub async fn members_list(space_id: String, state: State<'_, AppState>) -> Result<Vec<MemberInfo>, VeilError> {
    ...
    let db = state.db.read().await;
    let mut rows = db.list_space_members(&space_id).unwrap_or_default();
    if rows.is_empty() {
        if let Ok(ch_members) = db.list_channel_members(&space_id) {
            if !ch_members.is_empty() {
                let mut member_rows = Vec::new();
                for uid in ch_members {
                    let prof = db.get_profile_by_id(&uid).ok().flatten();
                    let (uname, disp, av, status) = match prof {
                        Some((u, d, a, _, s)) => (u, d, a, s),
                        None => (format!("user-{}", &uid.to_string()[..8]), "Kullanıcı".to_string(), None, "offline".to_string()),
                    };
                    member_rows.push(crate::db::channels::MemberRow {
                        user_id: uid,
                        username: uname,
                        display_name: disp,
                        avatar_hash: av,
...
pub async fn spaces_join_public(
    ...
    {
        // FK güvenliği: `space_members.spaces(id)` kısıtı yüzünden yerelde satır
        // yoksa üyelik INSERT'i "local database error" verir — satırı önce onar.
        let space_missing = {
            let g = state.db.read().await;
            match g.get_space(&space_id) {
                Ok(Some(_)) => false,
                other => {
                    if let Err(e) = other {
                        tracing::error!("join: get_space failed: {}", e);
                    }
                    true
                }
            }
        ...
        if space_missing {
            if config::configured("VEILANON_SUPABASE_URL") {
                let network = state.network.read().await;
                let filter = format!("id=eq.{}&select=id,name,icon_hash,owner_id,banner_hash,description", space_id);
                let fetched = network.api.select::<serde_json::Value>("spaces", &filter, None, Some(1)).await.ok();
                drop(network);
                if let Some(rows) = fetched {
                    if let Some(s) = rows.first() {
                        let db = state.db.read().await;
                        let n = s.get("name").and_then(|v| v.as_str()).unwrap_or("Topluluk").to_string();
                        let icon = s.get("icon_hash").and_then(|v| v.as_str());
                        let owner_str = s.get("owner_id").and_then(|v| v.as_str()).unwrap_or("");
                        let owner_uuid = Uuid::parse_str(owner_str).unwrap_or(identity.id);
                        if let Err(e) = db.insert_space(&space_id, &n, icon, &owner_uuid) {
                            tracing::error!("join: insert_space repair failed: {}", e);
                        }
                        if let Some(b) = s.get("banner_hash").and_then(|v| v.as_str()) {
                            let _ = db.update_space(&space_id, None, None, Some(Some(b)), None);
                        }
                        if let Some(d) = s.get("description").and_then(|v| v.as_str()) {
                            let _ = db.update_space(&space_id, None, None, None, Some(Some(d)));
                        }
                    }
                }
            }
            let db = state.db.read().await;
            let repaired = db.get_space(&space_id).map(|s| s.is_some()).unwrap_or(false);
            if !repaired {
                if let Err(e) = db.insert_space(&space_id, "Topluluk", None, &identity.id) {
                    tracing::error!("join: placeholder insert_space failed: {}", e);
                    return Err(VeilError::InvalidInput("Topluluk yerel veritabanına eklenemedi. Uygulamayı yeniden başlatıp tekrar deneyin.".into()));
                }
            }
        }
        ...
        // FK güvenliği: `space_members.space_id` -> `spaces(id)` kısıtı yüzünden
        // spaces satırı yoksa INSERT "local database error" verir. Satırı son bir
        // kez garantiye al, sonra üyelik ekle; yine de hata olursa açık mesaj dön.
        let space_present = db.get_space(&space_id).map(|s| s.is_some()).unwrap_or(false);
        if !space_present {
            if let Err(e) = db.insert_space(&space_id, "Topluluk", None, &identity.id) {
                tracing::error!("join: pre-membership insert_space failed: {}", e);
            }
        }
        match db.add_space_member(&space_id, &identity.id) {
            Ok(_) => {}
            Err(e) => {
                tracing::warn!("join: add_space_member notice for space {}: {}", space_id, e);
                let _ = db.insert_space(&space_id, "Topluluk", None, &identity.id);
                let _ = db.add_space_member(&space_id, &identity.id);
            }
        }
    ...
    // 5. Supabase membership kaydı ve kanal üyelikleri
    if config::configured("VEILANON_SUPABASE_URL") {
        best_effort_insert(
            &state,
            "memberships",
        ...
        let db = state.db.read().await;
        if let Ok(channels) = db.get_channels_for_space(&space_id) {
            let network = state.network.read().await;
            for ch in channels {
                let _ = db.add_channel_member(&ch.id, &identity.id);
    ...
    let db = state.db.read().await;
    let updated_row = match db.get_space(&space_id)? {
        Some(row) => row,
        None => {
            let _ = db.insert_space(&space_id, "Topluluk", None, &identity.id);
            db.get_space(&space_id)?.ok_or(VeilError::InvalidInput("Topluluk bulunamadı".into()))?
