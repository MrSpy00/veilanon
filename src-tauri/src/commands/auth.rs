...
pub async fn update_profile(
    ...
    if config::configured("VEILANON_SUPABASE_URL") {
        ...
        // Ephemeral broadcast to notify other clients instantly (0ms)
        ...
        realtime.broadcast(serde_json::json!({
            ...
            "username": target_username,
            "display_name": display_name,
            "avatar_hash": target_avatar,
            "banner_hash": target_banner,
            "custom_status": input.custom_status.as_deref(),
        }));
    }
...
async fn resolve_image_bytes(path_or_data: &str) -> Result<(Vec<u8>, String), VeilError> {
    if path_or_data.starts_with("data:") {
        ...
        let bytes = B64
            .decode(b64)
            .map_err(|_| VeilError::InvalidInput("Geçersiz base64 medya verisi".into()))?;
        let ext = if bytes.starts_with(b"GIF87a") || bytes.starts_with(b"GIF89a") || header.contains("gif") {
            ...
            "png"
        } else if (bytes.len() > 8 && (&bytes[4..8] == b"ftyp" || &bytes[4..8] == b"moov")) || header.contains("mp4") {
            "mp4"
        } else if bytes.starts_with(b"\x1A\x45\xDF\xA3") || header.contains("webm") {
            "webm"
        } else {
            "png"
        };
        Ok((bytes, ext.to_string()))
    } else if path_or_data.starts_with("http://") || path_or_data.starts_with("https://") {
        let client = reqwest::Client::builder()
            .timeout(std::time::Duration::from_secs(12))
            .build()
            .map_err(VeilError::NetworkError)?;
        let resp = client.get(path_or_data)
            .send()
            .await
            .map_err(VeilError::NetworkError)?;
        ...
        let ext = if bytes.starts_with(b"GIF87a") || bytes.starts_with(b"GIF89a") || ct.contains("gif") || path_or_data.ends_with(".gif") {
            "gif"
        } else if (bytes.len() > 12 && &bytes[0..4] == b"RIFF" && &bytes[8..12] == b"WEBP") || ct.contains("webp") || path_or_data.ends_with(".webp") {
            "webp"
        } else if (bytes.len() > 8 && (&bytes[4..8] == b"ftyp" || &bytes[4..8] == b"moov")) || ct.contains("mp4") || path_or_data.ends_with(".mp4") {
            "mp4"
        } else if bytes.starts_with(b"\x1A\x45\xDF\xA3") || ct.contains("webm") || path_or_data.ends_with(".webm") {
            "webm"
        } else if bytes.starts_with(b"\xFF\xD8\xFF") || ct.contains("jpeg") || ct.contains("jpg") || path_or_data.ends_with(".jpg") || path_or_data.ends_with(".jpeg") {
            "jpg"
        } else if bytes.starts_with(b"\x89PNG") || ct.contains("png") || path_or_data.ends_with(".png") {
            "png"
        } else {
            "png"
        };
        ...
        let ext = if bytes.starts_with(b"GIF87a") || bytes.starts_with(b"GIF89a") {
            ...
            std::path::Path::new(path_or_data)
                ...
                .filter(|e| matches!(e.as_str(), "png" | "jpg" | "jpeg" | "gif" | "webp" | "mp4" | "webm"))
...
pub async fn set_avatar(path: String, state: State<'_, AppState>) -> Result<String, VeilError> {
    ...
    let (bytes, ext) = resolve_image_bytes(&path).await?;
    ...
    if config::configured("VEILANON_SUPABASE_URL") {
        ...
        let _ = network
            ...
            .await;
        let realtime = network.realtime.clone();
        realtime.broadcast(serde_json::json!({
            "type": "profile_updated",
            "user_id": identity_id.to_string(),
            "id": identity_id.to_string(),
    ...
    let _ = state.app.emit("user:updated", serde_json::json!({ "userId": identity_id.to_string(), "avatar_hash": hash }));
    info!("Avatar updated");
    Ok(hash)
}
...
pub async fn set_banner(path: String, state: State<'_, AppState>) -> Result<String, VeilError> {
    ...
    let _ = state.app.emit("user:updated", serde_json::json!({ "userId": identity_id.to_string(), "banner_hash": hash }));
    info!("Banner updated");
    Ok(hash)
}
...
pub async fn get_avatar(hash: String, state: State<'_, AppState>) -> Result<String, VeilError> {
    ...
    let mime = if bytes.starts_with(b"GIF87a") || bytes.starts_with(b"GIF89a") {
        "image/gif"
    } else if bytes.len() > 12 && &bytes[0..4] == b"RIFF" && &bytes[8..12] == b"WEBP" {
        "image/webp"
    } else if bytes.starts_with(b"\xFF\xD8\xFF") {
        ...
        match std::path::Path::new(safe_hash)
            .extension()
            .and_then(|e| e.to_str())
            .unwrap_or("png")
            .to_lowercase()
