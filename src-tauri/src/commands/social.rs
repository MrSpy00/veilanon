...

    {
        let peer_ids: Vec<Uuid> = {
            ...
            if let Ok(local_rows) = db.list_friends(&identity.id) {
                local_rows.iter().filter_map(|r| Uuid::parse_str(&r.user_id.to_string()).ok()).collect()
            } else { Vec::new() }
        };
                    ...
                    if let Some(uid_str) = p.get("user_id").and_then(|v| v.as_str()) {
                        if let Ok(uid) = Uuid::parse_str(uid_str) {
                            ...
                            if raw_status != "offline" && raw_status != "invisible" {
                                ...
                                if hb_str.is_empty() { status_str = "offline"; }
                                else if let Ok(hb_time) = chrono::DateTime::parse_from_rfc3339(hb_str) {
                                    if (now - hb_time.with_timezone(&chrono::Utc)).num_seconds() > 90 { status_str = "offline"; }
                                } else { status_str = "offline"; }
                            } else if raw_status == "invisible" { status_str = "offline"; }
...
pub async fn dm_list(state: State<'_, AppState>) -> Result<Vec<ChannelInfo>, VeilError> {
                        ...
                        if let (Some(cid_str), Some(cname)) = (
                            ...
                            if let Ok(cid) = Uuid::parse_str(cid_str) {
                                let ctype_str = ch.get("channel_type").and_then(|v| v.as_str()).unwrap_or("direct_message");
                                    ...
                                    // 1:1 DM adını ortak stored isimle (karşı tarafın kendi
                                    // perspektifinden yazdığı = bizim nick'imiz) ezdirmeyi engelle.
                                    let is_dm_one_to_one = matches!(ctype, ChannelType::DirectMessage);
                                    let mut final_name = cname.to_string();
                                    if is_dm_one_to_one {
                                        let existing_name: Option<String> = db
                                            ...
                                            .ok();
                                        let good = existing_name
                                            .as_deref()
                                            .map(|n| {
                                                let t = n.trim();
                                                !t.is_empty() && t != "Direkt Mesaj" && !(t.len() == 36 && t.contains('-'))
                                            })
                                            .unwrap_or(false);
                                        if good {
                                            final_name = existing_name.unwrap_or(cname.to_string());
                                        }
                                    }
                                    let channel = Channel {
                                        id: cid,
    ...
    let mut channels_missing_members: Vec<Uuid> = Vec::new();

    for row in &rows {
        let stored_type = row.channel_type.as_str();
        if stored_type == "direct_message" || stored_type == "dm" {
            match db.list_channel_members(&row.id) {
                Ok(members) => {
                    if let Some(peer) = members.iter().find(|m| **m != identity.id) {
                        let prof_ok = db
                            .get_profile_by_id(peer)
                            .ok()
                            .flatten()
                            .map(|p| !p.0.trim().is_empty() || !p.1.trim().is_empty())
                            .unwrap_or(false);
                        if !prof_ok {
                            missing_peers.push(*peer);
                        }
                    } else {
                        channels_missing_members.push(row.id);
                    }
                }
                Err(_) => channels_missing_members.push(row.id),
            }
        }
    }
    drop(db);

    // Peer üye satırı yerelde hiç yoksa (ör. karşı taraf kanalı oluşturduysa)
    // Supabase'den üyeleri çek ve yerel üyeliği tamamla — aksi halde isim,
    // karşı cihazın yazdığı stored name'e düşer ve kendi nickin görünebilir.
    if !channels_missing_members.is_empty() && config::configured("VEILANON_SUPABASE_URL") {
        let network = state.network.read().await;
        let mut needs_self_member: Vec<Uuid> = Vec::new();
        for cid in &channels_missing_members {
            let cm_filter = format!("channel_id=eq.{}&select=user_id", cid);
            if let Ok(cm_rows) = network.api.select::<serde_json::Value>("channel_members", &cm_filter, None, Some(20)).await {
                let db = state.db.read().await;
                ...
                if !found_peer {
                    needs_self_member.push(*cid);
                }
            } else {
                needs_self_member.push(*cid);
            }
        }
        for cid in needs_self_member {
            let _ = network.api.upsert(
                "channel_members",
                &serde_json::json!({
    ...
    let db = state.db.read().await;
    let my_display = identity.display_name.trim().to_lowercase();
    let my_username = identity.username.trim().to_lowercase();
    let is_placeholder_name = |n: &str| {
        let t = n.trim();
        t.is_empty()
            || t == "Direkt Mesaj"
            || (t.len() == 36 && t.contains('-'))
    ...
    for row in rows {
        ...
        if stored_type == "direct_message" || stored_type == "dm" {
            ...
            // Guard: 1:1 DM adı asla kendi nick'imiz olamaz. Karşı taraf kanalı
            // kendi perspektifinden (peer=ben) oluşturduysa stored name bize
            // düşer — placeholder'a çevir ki buga girmesin.
            let name_l = info.name.trim().trim_start_matches('@').to_lowercase();
            if (!my_display.is_empty() && name_l == my_display)
                || (!my_username.is_empty() && name_l == my_username)
            {
                info.name = "Direkt Mesaj".to_string();
            }
            // Çözülen gerçek peer adını yerel satıra geri yaz — tüm okuyucular
            // aynı adı görür, flicker yok olur. Placeholder'lar asla yazılmaz.
            let resolved = info.name.trim().to_string();
            if !is_placeholder_name(&resolved) {
                let _ = db.execute(
                    "UPDATE channels SET name = ?1 WHERE id = ?2",
                    rusqlite::params![resolved, row.id.to_string()],
                );
            }
        }
        result.push(info);
    ...
    if !is_self && config::configured("VEILANON_SUPABASE_URL") {
        ...
        if let Ok(u_rows) = network.api.select::<serde_json::Value>("users", &u_filter, None, Some(1)).await {
            if let Some(u) = u_rows.first() {
                ...
                let bio = u.get("bio").and_then(|v| v.as_str()).filter(|s| !s.is_empty());
                let db = state.db.read().await;
                let _ = db.upsert_profile(&target, uname, disp, av, None, None, ban, bio, cs);
                if let Some(b) = ban {
        ...
        let f_filter = format!("or=(and(user_id.eq.{},friend_id.eq.{}),and(user_id.eq.{},friend_id.eq.{}))", identity.id, target, target, identity.id);
        if let Ok(f_rows) = network.api.select::<serde_json::Value>("friendships", &f_filter, None, Some(1)).await {
            let db = state.db.read().await;
            if let Some(r) = f_rows.first() {
                let r_user = r.get("user_id").and_then(|v| v.as_str()).unwrap_or("");
                let r_status = r.get("status").and_then(|v| v.as_str()).unwrap_or("pending");
                if r_user == identity.id.to_string() {
                    let local_status = if r_status == "accepted" { "friends" } else if r_status == "blocked" { "blocked" } else { "pending_outgoing" };
                    let _ = db.upsert_friend(&identity.id, &target, local_status);
                } else {
                    let local_status = if r_status == "accepted" { "friends" } else if r_status == "blocked" { "blocked" } else { "pending_incoming" };
                    let _ = db.upsert_friend(&identity.id, &target, local_status);
                }
