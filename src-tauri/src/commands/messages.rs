...
/// Encrypt a message with the DM ratchet; returns (ciphertext_b64, iv_b64, header_json, message_key).
async fn ratchet_encrypt(
    state: &AppState,
    channel_id: &uuid::Uuid,
    plaintext: &[u8],
) -> VeilResult<(String, String, String, [u8; 32])> {
    let (mut ratchet, peer) = load_or_create_ratchet(state, channel_id).await?;
    let (header, ciphertext, nonce, msg_key) = ratchet.encrypt_with_key(plaintext)?;
    let header_json =
    ...
    Ok((B64.encode(&ciphertext), B64.encode(&nonce), header_json, msg_key))
}
...
fn ratchet_decrypt(
    ...
    // 1. Cached per-message key first (fast path for previously decrypted or self-sent messages).
    if let Ok(Some(key)) = db.get_message_key(&msg.id, Some(db_key)) {
        if let (Ok(ct), Ok(nonce)) = (B64.decode(&msg.ciphertext), B64.decode(&msg.iv)) {
            if let Ok(plaintext) = decrypt_aes_gcm(&key, &ct, &nonce) {
    ...
    // If the message header has our own dh_public, this message was sent by ourselves.
    // If not found in message_key cache, don't perform ratchet_dh which would corrupt state.
    if header.dh_public == ratchet.dh_public {
        return None;
    }
...
async fn decrypt_batch(
    ...
    if is_dm {
        match load_or_create_ratchet(state, channel_id).await {
            Ok(session) => ratchet = Some(session),
            Err(e) => debug!("DM ratchet unavailable ({}); per-message key fallback", e),
        }
    } else if is_e2ee {
        match super::mls::load_session(state, channel_id).await {
            Ok(s) => mls_session = Some(s),
            Err(e) => debug!("MLS session unavailable ({}); per-message key fallback", e),
        }
    }
    ...
    for msg in messages {
        if let Some(exp) = msg.disappears_at {
        ...
        let content_str = if is_dm {
            ...
            if s.is_none() {
                s = decrypt_message_content(&db_key, &msg);
            }
            s
        } else if is_e2ee {
            let mut s = mls_session.as_mut().and_then(|s| {
                let ct = B64.decode(&msg.ciphertext).ok()?;
        ...
        let content = if let Some(c) = content_str {
            c
        } else if !msg.attachments.is_empty() || msg.deleted_at.is_some() {
            String::new()
        } else if (is_e2ee || is_dm) && (!msg.ciphertext.is_empty() || msg.crypto_meta.is_some()) {
            let is_self = identity.as_ref().map(|i| i.id == msg.sender_id).unwrap_or(false);
            if is_self {
                String::new()
            } else {
                "[Şifreli mesaj — anahtar senkronizasyonu bekleniyor]".to_string()
            }
        } else {
            String::new()
        };
...
pub async fn send_message(
    ...
    let (ciphertext_b64, iv_b64, crypto_meta, ratchet_key) = if is_dm {
        match ratchet_encrypt(&state, &channel_id, input.content.as_bytes()).await {
            Ok((ct, nonce, meta, mk)) => (ct, nonce, Some(meta), Some(mk)),
    ...
    // Store in local DB (ciphertext only)
    {
        ...
        if let Some(mk) = ratchet_key {
            if let Some(db_key) = state.get_db_key().await {
                let _ = db.save_message_key(&message_id, &mk, Some(&db_key));
            }
        }
    }
    ...
    // Broadcast instant signal over realtime so all connected peers sync within milliseconds
    if config::configured("VEILANON_SUPABASE_URL") {
...
fn decrypt_message_content(db_key: &[u8; 32], msg: &Message) -> Option<String> {
    if let Some(ref c) = msg.content {
        if !c.is_empty() && !c.starts_with("[Şifreli") {
            return Some(c.clone());
        }
    }
...
pub async fn edit_message(
    ...
    drop(db);
    let (ciphertext_b64, iv_b64, crypto_meta, edit_msg_key) = if is_dm {
        let (ct, nonce, meta, mk) = ratchet_encrypt(&state, &msg.channel_id, new_content.as_bytes()).await?;
        (ct, nonce, Some(meta), Some(mk))
    } else if is_e2ee {
...

    {
        ...
        if let Some(mk) = edit_msg_key {
            let _ = db.save_message_key(&msg_uuid, &mk, Some(&_db_key));
        } else {
...
pub async fn delete_message(
    ...
    let channel_id = {
        let db = state.db.read().await;
        let msg = db.get_message(&msg_uuid)?;
        let ch_id = msg.as_ref().map(|m| m.channel_id);
        let _ = db.soft_delete_message(&msg_uuid);
        let _ = db.hard_delete_messages(&[message_id.clone()]);
        ch_id
    };
...
pub async fn sync_messages(
    ...
    // Local clear gate: rows older than the last local "clear history" are
    // ignored even when the remote tombstone sweep failed (RLS).
    let cleared_at: i64 = {
        let db = state.db.read().await;
        db.get_channel_cleared_at(&channel_uuid)
    };
    ...
    let rows: Vec<RemoteRow> = {
        let network = state.network.read().await;
        let filter = format!("channel_id=eq.{}&deleted_at=is.null&select=id,channel_id,sender_id,sender_device_id,ciphertext,iv,crypto_meta,message_type,attachments,reactions,schema_version,client_created_at,edited_at,disappears_at,reply_to_id,pinned,deleted_at", channel_uuid);
        network.api.select("messages", &filter, Some("client_created_at.asc"), Some(200)).await?
    };

    // Tombstone pass: propagate peer deletions/clears that we can see.
    {
        let network = state.network.read().await;
        let t_filter = format!("channel_id=eq.{}&deleted_at=not.is.null&select=id", channel_uuid);
        if let Ok(tombstones) = network.api.select::<serde_json::Value>("messages", &t_filter, None, Some(200)).await {
            let deleted_ids: Vec<String> = tombstones
                .iter()
                .filter_map(|t| t.get("id").and_then(|v| v.as_str()).map(str::to_string))
                .collect();
            if !deleted_ids.is_empty() {
                let db = state.db.read().await;
                let _ = db.hard_delete_messages(&deleted_ids);
            }
        }
    }
    ...
    // Local rows by id so existing rows can be compared and updated (edits,
    // pins, reactions) instead of being skipped forever.
    let local_map: std::collections::HashMap<String, Message> = {
        let db = state.db.read().await;
        db.get_all_channel_messages(&channel_uuid)
            .unwrap_or_default()
            .into_iter()
    ...
    for row in rows {
        ...
        let disappears_ts = row.get("disappears_at").and_then(|v| v.as_str())
            .and_then(|s| chrono::DateTime::parse_from_rfc3339(s).ok())
            .map(|dt| dt.with_timezone(&chrono::Utc));
        let remote_edited = row.get("edited_at").and_then(|v| v.as_str())
            .and_then(|s| chrono::DateTime::parse_from_rfc3339(s).ok())
            .map(|dt| dt.with_timezone(&chrono::Utc));
        ...
        // Skip rows older than the last local clear.
        if cleared_at > 0 && created_ts.timestamp() <= cleared_at {
            continue;
        }

        // Skip expired disappearing messages and purge from local DB
        if let Some(exp) = disappears_ts {
            if exp <= chrono::Utc::now() {
                let db = state.db.read().await;
                let _ = db.hard_delete_messages(&[id.to_string()]);
                continue;
            }
        }

        // Already-known row: re-push only when the remote copy changed (edit,
        // pin, reaction, attachment) so those updates propagate in real time.
        if let Some(local) = local_map.get(id_str) {
            let changed = local.edited_at.map(|t| t.timestamp()) != remote_edited.map(|t| t.timestamp())
                || local.pinned != pinned
                || local.ciphertext != ciphertext
                || local.reactions != reactions
            ...
            };
            {
                ...
                if let Err(e) = db.insert_message(&message) {
                    debug!("Sync update insert skipped: {}", e);
                    continue;
                }
            }
            fresh.push(message);
    ...
    info!("Synced {} new message(s) for channel", fresh.len());
    let mut responses = decrypt_batch(&state, &channel_uuid, fresh).await?;
    // Veritabanı en yeni mesajı ilk sırada (DESC) döndürür. Sohbet akışında mesajların
    // kronolojik (en eski -> en yeni) dizilmesi için listeyi tersine çeviriyoruz.
    responses.reverse();
    Ok(responses)
    ...
    for peer_id in peer_ids {
        ...
        for (msg_id, channel_id, content, _msg_type, reply_to_id, attachments, disappears_at) in pending {
            match ratchet_encrypt(state, &channel_id, content.as_bytes()).await {
                Ok((ciphertext_b64, iv_b64, crypto_meta, mk)) => {
...

                    {
                        ...
                        if let Some(db_key) = state.get_db_key().await {
                            let _ = db.save_message_key(&msg_id, &mk, Some(&db_key));
                        }
                    }
