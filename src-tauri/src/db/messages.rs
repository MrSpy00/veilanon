...
impl Database {
    ...
    /// Fetch every stored row for a channel (unlimited) — used by sync to
    /// compare remote rows against local state and detect edits/reactions.
    pub fn get_all_channel_messages(&self, channel_id: &Uuid) -> VeilResult<Vec<Message>> {
        let channel_str = channel_id.to_string();
        self.query_map(
            r#"SELECT id, channel_id, sender_id, sender_device_id, ciphertext, iv,
                              message_type, status, reply_to_id, pinned, reactions, attachments,
    ...
    pub fn clear_channel_messages(&self, channel_id: &Uuid) -> VeilResult<()> {
        ...
        self.execute(
            ...
            params![now, channel_id.to_string()],
        )?;
        self.record_channel_clear(channel_id, now)?;
        Ok(())
    }

    /// Persist the moment a channel was locally cleared. `sync_messages` skips
    /// remote rows older than this even if the server-side bulk tombstone was
    /// blocked (RLS), so "clear history" never visually reverts.
    pub fn record_channel_clear(&self, channel_id: &Uuid, ts: i64) -> VeilResult<()> {
        self.execute(
            "INSERT INTO channel_clears (channel_id, cleared_at) VALUES (?1, ?2)
             ON CONFLICT(channel_id) DO UPDATE SET cleared_at = MAX(cleared_at, excluded.cleared_at)",
            params![channel_id.to_string(), ts],
        )?;
        Ok(())
    }

    /// Latest local clear timestamp for a channel (0 when never cleared).
    pub fn get_channel_cleared_at(&self, channel_id: &Uuid) -> i64 {
        self.query_row(
            "SELECT cleared_at FROM channel_clears WHERE channel_id = ?1",
    ...
    /// Hard-delete locally-known rows that were deleted remotely.
    pub fn hard_delete_messages(&self, ids: &[String]) -> VeilResult<usize> {
        let mut n = 0;
        for id in ids {
            n += self.execute(
